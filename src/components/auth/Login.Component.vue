...
import AuthService from "src/services/auth/loginAuth";
      ...
      try {
        ...
        if (response.data.status_code != 200) {
          ...
          if (response.data.message == "Cannot login, wrong password account") {
            this.showNotif("Incorrect password. Please check and try again.", "negative");
            return;
          }

          else if (response.data.message == "Cannot login, wrong email account") {
            this.showNotif(
              "Email not found or not registered. Please contact the website administrator for more information.",
              "negative"
            );
            return;
          }
